-- SQLite schema for monitoring

CREATE TABLE IF NOT EXISTS monitor_meta (
  k TEXT PRIMARY KEY,
  v TEXT NOT NULL
) WITHOUT ROWID;

CREATE TABLE IF NOT EXISTS monitor_migrations (
  version INTEGER PRIMARY KEY,
  applied_ts_ms INTEGER NOT NULL
);

CREATE TABLE IF NOT EXISTS pair_registry (
  pair_id INTEGER PRIMARY KEY,
  watchlist_version INTEGER NOT NULL,
  status TEXT NOT NULL,
  root_market_title TEXT,
  polymarket_event_title TEXT,
  opinion_market_id INTEGER,
  opinion_yes_token_id TEXT,
  opinion_no_token_id TEXT,
  polymarket_market_id TEXT,
  polymarket_yes_token_id TEXT,
  polymarket_no_token_id TEXT,
  created_local_ts_ms INTEGER NOT NULL
);

CREATE INDEX IF NOT EXISTS idx_pair_registry_status
  ON pair_registry(status, watchlist_version);

CREATE TABLE IF NOT EXISTS pair_token_overrides (
  pair_id INTEGER PRIMARY KEY,
  mapping_overridden_flag INTEGER NOT NULL DEFAULT 0,
  opinion_yes_token_id_override TEXT,
  opinion_no_token_id_override TEXT,
  polymarket_yes_token_id_override TEXT,
  polymarket_no_token_id_override TEXT,
  updated_local_ts_ms INTEGER NOT NULL
);

CREATE TABLE IF NOT EXISTS token_registry (
  token_key TEXT PRIMARY KEY,
  venue TEXT NOT NULL,
  external_token_id TEXT,
  market_id TEXT,
  outcome_side INTEGER,
  token_side TEXT,
  first_seen_local_ts_ms INTEGER NOT NULL,
  last_seen_local_ts_ms INTEGER NOT NULL
) WITHOUT ROWID;

CREATE INDEX IF NOT EXISTS idx_token_registry_venue_market
  ON token_registry(venue, market_id);

CREATE TABLE IF NOT EXISTS connection_health_1s (
  venue TEXT NOT NULL,
  bar_second INTEGER NOT NULL,
  ws_state INTEGER NOT NULL,
  reconnect_count INTEGER NOT NULL,
  last_msg_age_ms INTEGER,
  heartbeat_sent INTEGER,
  heartbeat_fail INTEGER,
  dropped_events INTEGER NOT NULL DEFAULT 0,
  notes TEXT,
  PRIMARY KEY (venue, bar_second)
) WITHOUT ROWID;

CREATE TABLE IF NOT EXISTS clock_offsets (
  venue TEXT NOT NULL,
  local_ts_ms INTEGER NOT NULL,
  exchange_ts_ms INTEGER NOT NULL,
  sample_offset_ms INTEGER NOT NULL,
  smoothed_offset_ms INTEGER NOT NULL,
  delta_clamped_ms INTEGER NOT NULL,
  PRIMARY KEY (venue, local_ts_ms)
) WITHOUT ROWID;

CREATE INDEX IF NOT EXISTS idx_clock_offsets_venue_time
  ON clock_offsets(venue, local_ts_ms);

CREATE TABLE IF NOT EXISTS raw_events (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  venue TEXT NOT NULL,
  token_key TEXT NOT NULL,
  pair_id INTEGER,
  token_side TEXT,
  kind TEXT NOT NULL,
  channel TEXT,
  exchange_ts_ms INTEGER NOT NULL,
  local_ts_ms INTEGER NOT NULL,
  exchange_ts_source INTEGER NOT NULL,
  obs_latency_ms INTEGER,
  out_of_order_flag INTEGER NOT NULL DEFAULT 0,
  ts_missing_flag INTEGER NOT NULL DEFAULT 0,
  ts_anomaly_flag INTEGER NOT NULL DEFAULT 0,
  trade_side_missing_flag INTEGER NOT NULL DEFAULT 0,
  venue_hash TEXT,
  payload_json TEXT
);

CREATE INDEX IF NOT EXISTS idx_raw_events_token_exchange
  ON raw_events(token_key, exchange_ts_ms);

CREATE INDEX IF NOT EXISTS idx_raw_events_pair_exchange
  ON raw_events(pair_id, exchange_ts_ms);

CREATE INDEX IF NOT EXISTS idx_raw_events_local
  ON raw_events(local_ts_ms);

CREATE TABLE IF NOT EXISTS orderbook_resync_log (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  venue TEXT NOT NULL,
  token_key TEXT NOT NULL,
  reason TEXT NOT NULL,
  trigger_local_ts_ms INTEGER NOT NULL,
  trigger_exchange_ts_ms INTEGER,
  venue_hash_before TEXT,
  venue_hash_after TEXT,
  local_topn_hash_before TEXT,
  local_topn_hash_after TEXT
);

CREATE INDEX IF NOT EXISTS idx_resync_token_time
  ON orderbook_resync_log(token_key, trigger_local_ts_ms);

CREATE TABLE IF NOT EXISTS bars_1s_token (
  venue TEXT NOT NULL,
  token_key TEXT NOT NULL,
  token_side TEXT NOT NULL,
  bar_second INTEGER NOT NULL,
  exchange_ts_source INTEGER NOT NULL,
  obs_latency_venue_p50_ms INTEGER,
  obs_latency_venue_p90_ms INTEGER,
  obs_latency_venue_p99_ms INTEGER,
  obs_latency_venue_max_ms INTEGER,
  obs_latency_est_p50_ms INTEGER,
  obs_latency_est_p90_ms INTEGER,
  obs_latency_est_p99_ms INTEGER,
  obs_latency_est_max_ms INTEGER,
  connection_health_flags INTEGER NOT NULL DEFAULT 0,
  bar_gap_flag INTEGER NOT NULL DEFAULT 0,
  bar_stale_flag INTEGER NOT NULL DEFAULT 0,
  staleness_ms INTEGER,
  event_count_1s INTEGER NOT NULL DEFAULT 0,
  ts_missing_count_1s INTEGER NOT NULL DEFAULT 0,
  ts_anomaly_count_1s INTEGER NOT NULL DEFAULT 0,
  book_resync_flag INTEGER NOT NULL DEFAULT 0,
  best_bid_px REAL,
  best_ask_px REAL,
  mid_px REAL,
  best_bid_px_state REAL,
  best_ask_px_state REAL,
  mid_px_state REAL,
  last_price REAL,
  micro_px REAL,
  spread_px REAL,
  tick_px REAL,
  spread_ticks INTEGER,
  top3_depth_bid_notional REAL,
  top3_depth_ask_notional REAL,
  bid_l1_notional REAL,
  bid_l2_notional REAL,
  bid_l3_notional REAL,
  ask_l1_notional REAL,
  ask_l2_notional REAL,
  ask_l3_notional REAL,
  top10_depth_bid_notional REAL,
  top10_depth_ask_notional REAL,
  depth_1pct_bid_notional REAL,
  depth_1pct_ask_notional REAL,
  imbalance_top10 REAL,
  air_pocket_bid_ticks INTEGER,
  air_pocket_ask_ticks INTEGER,
  book_updates_1s INTEGER NOT NULL DEFAULT 0,
  add_updates_1s INTEGER NOT NULL DEFAULT 0,
  cancel_updates_1s INTEGER NOT NULL DEFAULT 0,
  match_updates_1s INTEGER NOT NULL DEFAULT 0,
  remove_unknown_updates_1s INTEGER NOT NULL DEFAULT 0,
  trade_count_1s INTEGER NOT NULL DEFAULT 0,
  volume_shares_1s REAL NOT NULL DEFAULT 0,
  volume_notional_1s REAL NOT NULL DEFAULT 0,
  vwap_1s REAL,
  buy_notional_1s REAL NOT NULL DEFAULT 0,
  sell_notional_1s REAL NOT NULL DEFAULT 0,
  cvd_delta_1s REAL NOT NULL DEFAULT 0,
  trade_side_missing_count_1s INTEGER NOT NULL DEFAULT 0,
  ofi_1s REAL NOT NULL DEFAULT 0,
  ofi_250ms REAL NOT NULL DEFAULT 0,
  bid_delta_notional_250ms REAL NOT NULL DEFAULT 0,
  ask_delta_notional_250ms REAL NOT NULL DEFAULT 0,
  mid_slope_250ms REAL,
  ofi_10s REAL,
  ofi_z_30s REAL,
  levels_crossed_1s INTEGER NOT NULL DEFAULT 0,
  max_trade_notional_1s REAL NOT NULL DEFAULT 0,
  buy_notional_250ms REAL NOT NULL DEFAULT 0,
  sell_notional_250ms REAL NOT NULL DEFAULT 0,
  max_trade_notional_250ms REAL NOT NULL DEFAULT 0,
  depth_withdrawal_ratio REAL,
  liquidity_pull_flag INTEGER NOT NULL DEFAULT 0,
  venue_book_hash TEXT,
  local_topn_hash TEXT,
  PRIMARY KEY (venue, token_key, bar_second)
) WITHOUT ROWID;

CREATE INDEX IF NOT EXISTS idx_bars_1s_token_time
  ON bars_1s_token(venue, bar_second);
CREATE INDEX IF NOT EXISTS idx_bars_1s_token_pm_analysis
  ON bars_1s_token(
    token_key,
    bar_second,
    book_updates_1s,
    volume_notional_1s,
    top10_depth_bid_notional,
    top10_depth_ask_notional
  )
  WHERE venue = 'pm';
CREATE INDEX IF NOT EXISTS idx_bars_1s_token_pm_time_token_cover
  ON bars_1s_token(
    bar_second,
    token_key,
    book_updates_1s,
    volume_notional_1s
  )
  WHERE venue = 'pm';

CREATE TABLE IF NOT EXISTS bars_1s_pair (
  pair_id INTEGER NOT NULL,
  token_side TEXT NOT NULL,
  bar_second INTEGER NOT NULL,
  poly_bar_gap_flag INTEGER NOT NULL DEFAULT 0,
  opi_bar_gap_flag INTEGER NOT NULL DEFAULT 0,
  bar_gap_flag INTEGER NOT NULL DEFAULT 0,
  poly_bar_stale_flag INTEGER NOT NULL DEFAULT 0,
  opi_bar_stale_flag INTEGER NOT NULL DEFAULT 0,
  bar_stale_flag INTEGER NOT NULL DEFAULT 0,
  poly_staleness_ms INTEGER,
  opi_staleness_ms INTEGER,
  poly_best_bid REAL,
  poly_best_ask REAL,
  poly_mid REAL,
  poly_micro REAL,
  poly_best_bid_state REAL,
  poly_best_ask_state REAL,
  poly_mid_state REAL,
  opi_best_bid REAL,
  opi_best_ask REAL,
  opi_mid REAL,
  opi_best_bid_state REAL,
  opi_best_ask_state REAL,
  opi_mid_state REAL,
  opi_last_price REAL,
  opi_micro REAL,
  opi_bid_l1_notional REAL,
  opi_bid_l2_notional REAL,
  opi_bid_l3_notional REAL,
  opi_ask_l1_notional REAL,
  opi_ask_l2_notional REAL,
  opi_ask_l3_notional REAL,
  poly_fee_bps INTEGER,
  opi_fee_bps INTEGER,
  gas_cost REAL,
  network_congestion_flag INTEGER NOT NULL DEFAULT 0,
  gross_spread_buy REAL,
  gross_spread_sell REAL,
  true_net_spread_buy_opinion REAL,
  true_net_spread_sell_opinion REAL,
  arb_mode_flag INTEGER NOT NULL DEFAULT 0,
  directional_mode_flag INTEGER NOT NULL DEFAULT 0,
  poly_cvd_30s REAL,
  poly_buy_sell_ratio_30s REAL,
  price_pressure_index REAL,
  poly_ofi_250ms REAL,
  poly_buy_notional_1s REAL,
  poly_sell_notional_1s REAL,
  poly_max_trade_notional_1s REAL,
  poly_buy_notional_250ms REAL,
  poly_sell_notional_250ms REAL,
  poly_max_trade_notional_250ms REAL,
  poly_bid_delta_notional_250ms REAL,
  poly_ask_delta_notional_250ms REAL,
  poly_mid_slope_250ms REAL,
  poly_obs_latency_p99_ms INTEGER,
  opi_obs_latency_p99_ms INTEGER,
  rolling_corr_5m REAL,
  estimated_lag_s INTEGER,
  lag_confidence REAL,
  beta_1m REAL,
  follow_score_raw REAL,
  follow_score_adj REAL,
  PRIMARY KEY (pair_id, token_side, bar_second)
) WITHOUT ROWID;

CREATE INDEX IF NOT EXISTS idx_bars_1s_pair_time
  ON bars_1s_pair(bar_second);
CREATE INDEX IF NOT EXISTS idx_bars_1s_pair_opi_mid_cover
  ON bars_1s_pair(pair_id, token_side, bar_second, opi_mid_state)
  WHERE opi_mid_state IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_bars_1s_pair_opi_price_cover
  ON bars_1s_pair(pair_id, token_side, bar_second, opi_last_price)
  WHERE opi_last_price IS NOT NULL;

CREATE TABLE IF NOT EXISTS fee_schedule (
  venue TEXT NOT NULL,
  fee_bps INTEGER NOT NULL,
  effective_from_ts_ms INTEGER NOT NULL,
  effective_to_ts_ms INTEGER,
  PRIMARY KEY (venue, effective_from_ts_ms)
) WITHOUT ROWID;

CREATE TABLE IF NOT EXISTS chain_metrics (
  chain TEXT NOT NULL,
  local_ts_ms INTEGER NOT NULL,
  base_fee REAL,
  priority_fee REAL,
  gas_used_est REAL,
  tx_cost_est REAL,
  congestion_flag INTEGER NOT NULL DEFAULT 0,
  PRIMARY KEY (chain, local_ts_ms)
) WITHOUT ROWID;

CREATE INDEX IF NOT EXISTS idx_chain_metrics_time
  ON chain_metrics(local_ts_ms);

CREATE TABLE IF NOT EXISTS poly_shock_events (
  shock_id INTEGER PRIMARY KEY AUTOINCREMENT,
  token_key TEXT NOT NULL,
  pair_id INTEGER,
  token_side TEXT NOT NULL,
  shock_type TEXT NOT NULL,
  direction INTEGER NOT NULL,
  noise_flag INTEGER NOT NULL DEFAULT 0,
  start_ts_ms INTEGER NOT NULL,
  trigger_ts_ms INTEGER NOT NULL,
  peak_ts_ms INTEGER NOT NULL,
  magnitude REAL NOT NULL,
  context_json TEXT,
  obs_latency_ms INTEGER,
  connection_health_flags INTEGER NOT NULL DEFAULT 0,
  status TEXT NOT NULL DEFAULT 'PENDING',
  peak_finalized INTEGER NOT NULL DEFAULT 0
);

CREATE INDEX IF NOT EXISTS idx_shock_time
  ON poly_shock_events(trigger_ts_ms);

CREATE INDEX IF NOT EXISTS idx_shock_token_time
  ON poly_shock_events(token_key, trigger_ts_ms);

CREATE INDEX IF NOT EXISTS idx_shock_type_time
  ON poly_shock_events(shock_type, trigger_ts_ms);

CREATE INDEX IF NOT EXISTS idx_shock_pair_time
  ON poly_shock_events(pair_id, token_side, trigger_ts_ms);

CREATE TABLE IF NOT EXISTS opinion_response (
  shock_id INTEGER NOT NULL,
  horizon_s INTEGER NOT NULL,
  sample_valid_flag INTEGER NOT NULL DEFAULT 1,
  move_flag INTEGER NOT NULL DEFAULT 0,
  first_move_lag_s INTEGER,
  opi_return REAL,
  opi_price_delta REAL,
  opi_volume_sum REAL,
  opi_cvd_sum REAL,
  opi_spread_change REAL,
  opi_depth_change REAL,
  opi_staleness_at_t0_ms INTEGER,
  opi_liquidity_pull_flag_at_t0 INTEGER,
  true_net_spread_at_t0 REAL,
  arb_mode_flag_at_t0 INTEGER,
  estimated_lag_s_at_t0 INTEGER,
  lag_confidence_at_t0 REAL,
  obs_latency_poly_p99_at_t0 INTEGER,
  obs_latency_opi_p99_at_t0 INTEGER,
  PRIMARY KEY (shock_id, horizon_s)
) WITHOUT ROWID;

CREATE INDEX IF NOT EXISTS idx_opinion_response_horizon
  ON opinion_response(horizon_s, shock_id, sample_valid_flag);

CREATE TABLE IF NOT EXISTS trade_audit (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  event_ts_ms INTEGER NOT NULL,
  pair_id INTEGER NOT NULL,
  token_side TEXT NOT NULL,
  phase TEXT NOT NULL,
  direction INTEGER NOT NULL,
  reason TEXT,
  pm_move REAL,
  pm_mom REAL,
  ofi_250ms REAL,
  edge_raw REAL,
  edge_norm REAL,
  entry_price REAL,
  entry_qty REAL,
  exit_price_target REAL,
  exit_price REAL,
  gas_est_usd REAL,
  min_profit_usd REAL,
  book_decay REAL,
  book_support REAL,
  book_scale REAL,
  retrace_ratio REAL,
  inv_ratio REAL,
  inv_factor REAL,
  decay_factor REAL,
  pnl_usd_net REAL,
  holding_ms INTEGER,
  mae REAL,
  mfe REAL,
  detail_json TEXT
);

CREATE INDEX IF NOT EXISTS idx_trade_audit_time
  ON trade_audit(event_ts_ms);

CREATE INDEX IF NOT EXISTS idx_trade_audit_pair_time
  ON trade_audit(pair_id, token_side, event_ts_ms);
